...
The goal is to demonstrate **interoperability-oriented backend engineering** with professional software practices: clean architecture, explicit technical decision records (ADRs), API documentation, testing, and reproducible synthetic datasets. The application is intentionally designed to remain small, iterative, and easy to evaluate by a reviewer.

### Why this is healthcare/biotech-specific
...
- No FastAPI/SQLAlchemy in domain layer

---

### Phase 2 — ADR baseline and architectural skeleton (Issue)
**Objective:** Lock down foundational decisions and repository conventions.
- ADRs: repo structure, Python dependency management (Pipenv), API stack, FHIR-like scope, auth strategy, audit strategy
...
### Phase 3 — Backend foundation (Issue)
**Objective:** Provide executable API skeleton with persistence and configuration.
...
### Phase 4 — Clinical use-cases and endpoints (Issue)
...
- Audit trail recording on access/export

**Definition of Done**
- OpenAPI examples in `docs/api/examples/`
- Integration tests for main endpoints
- Audit events persisted and queryable (admin)

---

### Phase 5 — Synthetic dataset and demo scenarios (Issue)
...
### Phase 7 — Hardening and portfolio packaging (Issue)
